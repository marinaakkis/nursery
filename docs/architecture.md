...
interface LlmProvider {
  name: "http" | "rules";
  run(input: { messages: Message[]; tools: Tool[] }): AsyncIterable<Chunk>;
}
```

Две реализации, выбор — переменной окружения `LLM_PROVIDER`:

- **`http`** — внешняя модель по HTTP. Адрес, ключ и имя модели приходят из окружения. Если у модели есть tool-calling, используется он. Если нет — модель возвращает структурированное намерение (`{ tool, args }`), а вызывает инструмент наш код; для остальной системы путь тот же, меняется только источник намерения.
...
**Не тестируем и почему:** попиксельную вёрстку — глазами дешевле; внешнего провайдера модели — он вне нашего контроля, вместо него в тестах детерминированный; заглушки оплаты и уведомлений — тестировать нечего; доступность и контраст — проверяются по чек-листу вручную.
