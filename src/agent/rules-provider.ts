...
const CARE_RULES: Rule[] = [
  {
    // Одно и то же покупатель называет десятком способов: «простой»,
    // «лёгкий», «неприхотливый», «некогда возиться». Все они про одно.
    keys: [
      "без ухода", "не ухаживать", "некогда", "по будням", "неприхотлив",
      "минимум ухода", "забывчив", "прост", "легк", "лёгк", "несложн",
      "не требовател", "нетребовател", "чтобы рос сам", "растет сам", "растёт сам",
    ],
    apply: (f) => (f.care = "low"),
    reading: "уход — легко",
  },
  {
    keys: ["средний уход", "умеренн", "средне", "не самый простой"],
    apply: (f) => (f.care = "medium"),
    reading: "уход — средне",
  },
  {
    keys: [
      "готов ухаживать", "люблю возиться", "не боюсь ухода", "требовател",
      "сложн", "капризн", "готов повозиться", "много времени",
    ],
    apply: (f) => (f.care = "high"),
    reading: "уход — сложно, и это не пугает",
  },
];
