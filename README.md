...
Next.js (App Router) · TypeScript · PostgreSQL · Drizzle. Агентный цикл написан руками,
без SDK. Один процесс приложения плюс база.
...
Почему цикл рукописный и как подключена внешняя модель — [ADR 0003](memory/decisions/0003-agent-loop-and-http-provider.md).
...
`src/agent/http-provider.ts` ходит в gateway формата OpenAI chat completions: один POST,
модель и адрес из окружения, ключ в заголовке. Системный промпт собирается из реестра —
`allTools()` отдаёт имя, описание и zod-схему параметров каждого инструмента, — а ответ
модели проходит zod-схему `AgentPlan`. Инструменты провайдер не вызывает: модель возвращает
намерение, вызывает — исполнитель, через те же ворота подтверждения.

1. Задать переменные окружения: `LLM_PROVIDER=http`, `LLM_BASE_URL`, `LLM_API_KEY`,
   `LLM_MODEL`. Порядок включения на стенде и что проверить после — в `DEPLOY.md`.
2. Больше ничего не трогать. Исполнитель, ворота подтверждения, карточка «что сделал
   и почему» и экраны о смене провайдера не знают; индикатор на `/assistant` читает
   режим из окружения сам.

Любая неудача — нет переменных, сеть, таймаут 15 с, ответ не по схеме — это отказ
с причиной на экране, а не тихий откат на правила. Ключ модели живёт только в переменной
